...
if [ $MACHINE = "Mac" ]; then
 if leaks --atExit -- ./ELFTester > $STD_TEMPFILE  2> $ERR_TEMPFILE; then
    ALL_PASS=0
  else
    ALL_PASS=1
  fi
...
if [ $MACHINE = "Mac" ]; then
  if grep -qF "0 leaks for 0 total leaked bytes." $STD_TEMPFILE; then
      echo -e "[ \e[32mPASS\e[39m ]: Memory leak check"
  else
      echo -e "[ \e[31mFAIL\e[39m ]: Memory leak check"
      ALL_PASS=1
  fi
fi
